...
// ── Types ─────────────────────────────────────────────────────────────────────
type Tab = 'overview' | 'users' | 'courses' |'sections' |'lessons' |'quizzes' |'enrollments' |'categories' |'reviews' | 'stats' | 'payments';
...
const ROLE_LABEL: Record<string, string> = {
  student: 'Học viên', instructor: 'Giảng viên', admin: 'Admin',
};
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate, onLogout }) => {
  ...
  interface SectionForm { title: string; description: string; order_index: number | string; course: string; }
  const EMPTY_SECTION: SectionForm = { title: '', description: '', order_index: 0, course: '' };
  ...
  const EMPTY_QUIZ: QuizForm = {
    ...
    pass_score: 70, time_limit: 0, max_attempts: 0,
  };
  const EMPTY_QUESTION: QuestionForm = {
    content: '', question_type: 'single', points: 1,
  ...
  const [categoryModal,      setCategoryModal]      = useState<'add' | 'edit' | 'delete' | null>(null);
  const [selectedCategory,   setSelectedCategory]   = useState<any>(null);
  const [categoryForm,       setCategoryForm]       = useState({ name: '', description: '', is_pinned: false, pin_order: 0 });
  ...
  const openViewUser  = (u: any) => { setSelectedUser(u); setUserViewModal(true); };
  const closeViewUser = ()       => { setUserViewModal(false); setSelectedUser(null); };
  ...
  const fetchQuestions = useCallback(async (quizId: string) => {
    setLoadingQ(true);
    try {
      const res = await fetch(`${API}/api/quizzes/${quizId}/questions/`, { headers: authHeader() });
  ...
  const fetchEnrollments = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/enrollments/admin/`, { headers: authHeader() });
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/courses/categories/`, { headers: authHeader() });
  ...
  const fetchInstructors = useCallback(async () => {
    try {
      const res = await fetch(`${API}/api/auth/users/?role=instructor`, { headers: authHeader() });
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/courses/reviews/admin/`, { headers: authHeader() });
  ...
  const openDeleteReview = (r: any) => { setSelectedReview(r); setReviewModal('delete'); };
  ...
  const handleDeleteReview = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/courses/reviews/admin/${selectedReview.id}/`, {
        method: 'DELETE', headers: authHeader(),
      });
      if (res.ok || res.status === 204) { fetchReviews(); closeReviewModal(); }
    } catch {}
  ...
  const openAddCategory = () => {
    setCategoryForm({ name: '', description: '', is_pinned: false, pin_order: 0 });
    setCategoryError('');
    setSelectedCategory(null);
    setCategoryModal('add');
  };
  const openEditCategory = (cat: any) => {
    setCategoryForm({ name: cat.name ?? '', description: cat.description ?? '',is_pinned: cat.is_pinned ?? false, pin_order: cat.pin_order ?? 0, });
    setCategoryError('');
  ...
  const handleSaveCategory = async () => {
    if (!categoryForm.name.trim()) {
      setCategoryError('Tên danh mục không được để trống.'); return;
    }
    setSavingCategory(true); setCategoryError('');
    try {
      const url    = categoryModal === 'add'
        ? `${API}/api/courses/categories/`
        : `${API}/api/courses/categories/${selectedCategory.id}/`;
      const method = categoryModal === 'add' ? 'POST' : 'PATCH';
      const res    = await fetch(url, {
        method, headers: jsonH(),
        body: JSON.stringify({ name: categoryForm.name.trim(), description: categoryForm.description.trim(), is_pinned:   categoryForm.is_pinned, pin_order:   Number(categoryForm.pin_order), }),
      });
      if (res.ok) { fetchCategories(); closeCategoryModal(); }
      else {
        const err = await res.json().catch(() => ({}));
        setCategoryError(err?.detail ?? err?.name?.[0] ?? `Lỗi HTTP ${res.status}`);
      }
    } catch { setCategoryError('Lỗi kết nối. Vui lòng thử lại.'); }
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/courses/categories/${selectedCategory.id}/`, {
        method: 'DELETE', headers: authHeader(),
      });
      if (res.ok || res.status === 204) { fetchCategories(); closeCategoryModal(); }
      else setCategoryError('Xóa thất bại. Danh mục có thể đang được dùng.');
    } catch { setCategoryError('Lỗi kết nối.'); }
    setSavingCategory(false);
  };
  ...
  const fetchSections = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/courses/sections/`, { headers: authHeader() });
  ...
  useEffect(() => { fetchLessons(); }, [filterLessonSection]);
  ...
  const validate = (): boolean => {
    if (!String(courseForm.title).trim()) {
      setFormError('Tên khóa học không được để trống.'); return false;
    }
    if (Number(courseForm.price) < 0) {
      setFormError('Học phí không hợp lệ.'); return false;
    }
    if (
      Number(courseForm.discount_percent) < 0 ||
      Number(courseForm.discount_percent) > 100
    ) {
      setFormError('Phần trăm giảm giá phải từ 0 đến 100.'); return false;
    }
    return true;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (hasNewThumb) {
        ...
        fd.append('discount_percent', String(Number(courseForm.discount_percent)));
        ...
        if (courseForm.instructor) fd.append('instructor', courseForm.instructor);
        if (courseForm.thumbnail)  fd.append('thumbnail',  courseForm.thumbnail);
        if (courseForm.published_at) {
          fd.append('published_at', new Date(courseForm.published_at).toISOString());
        }
        body    = fd;
        headers = authHeader();
      } else {
        ...
        if (courseForm.published_at) {
          payload.published_at = new Date(courseForm.published_at).toISOString();
        } else {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API}/api/courses/admin/${selectedCourse.id}/`, {
        method: 'DELETE', headers: authHeader(),
      });
  ...
  const handleSaveSection = async () => {
    if (!String(sectionForm.title).trim()) {
      setSectionError('Tên chương không được để trống.'); return;
    }
    if (!sectionForm.course) {
      setSectionError('Vui lòng chọn khóa học.'); return;
    }
    ...
    try {
      ...
      const url    = sectionModal === 'add'
        ? `${API}/api/courses/sections/`
        : `${API}/api/courses/sections/${selectedSection.id}/`;
      const method = sectionModal === 'add' ? 'POST' : 'PATCH';
      const res    = await fetch(url, {
        method, headers: jsonH(), body: JSON.stringify(payload),
      });
      if (res.ok) { fetchSections(); closeSectionModal(); }
      else {
        const err = await res.json().catch(() => ({}));
        setSectionError(err?.detail ?? err?.title?.[0] ?? `Lỗi HTTP ${res.status}`);
      }
    } catch { setSectionError('Lỗi kết nối. Vui lòng thử lại.'); }
  ...
  const handleDeleteSection = async () => {
    ...
    setSavingSection(true);
    try {
      const res = await fetch(`${API}/api/courses/sections/${selectedSection.id}/`, {
        method: 'DELETE', headers: authHeader(),
      });
      if (res.ok || res.status === 204) { fetchSections(); closeSectionModal(); }
  ...
  const handleSaveLesson = async () => {
    if (!String(lessonForm.title).trim()) {
      setLessonError('Tên bài học không được để trống.'); return;
    }
    if (!lessonForm.section) {
      setLessonError('Vui lòng chọn chương.'); return;
    }
  ...
  const handleDeleteLesson = async () => {
    ...
    setSavingLesson(true);
    try {
      const res = await fetch(`${API}/api/courses/lessons/admin/${selectedLesson.id}/`, {
        method: 'DELETE', headers: authHeader(),
      });
      if (res.ok || res.status === 204) { fetchLessons(); closeLessonModal(); }
  ...
  const handleSaveQuiz = async () => {
    ...
    if (!String(quizForm.title).trim()) { setQuizError('Tên bài kiểm tra không được để trống.'); return; }
    ...
    try {
      ...
      const url    = quizModal === 'add' ? `${API}/api/quizzes/` : `${API}/api/quizzes/${selectedQuiz.id}/`;
      ...
      if (res.ok) { fetchQuizzes(); closeQuizModal(); }
        ...
        const err = await res.json().catch(() => ({}));
        setQuizError(err?.detail ?? err?.lesson?.[0] ?? `Lỗi HTTP ${res.status}`);
      }
    } catch { setQuizError('Lỗi kết nối.'); }
    setSavingQuiz(false);
  };
  const handleDeleteQuiz = async () => {
    ...
    setSavingQuiz(true);
    try {
      const res = await fetch(`${API}/api/quizzes/${selectedQuiz.id}/`, { method: 'DELETE', headers: authHeader() });
  ...
  const handleSaveQuestion = async () => {
    if (!String(questionForm.content).trim()) { setQuestionError('Nội dung câu hỏi không được để trống.'); return; }
    ...
    } catch { setQuestionError('Lỗi kết nối.'); }
    setSavingQ(false);
  };
  const handleDeleteQuestion = async () => {
    ...
    setSavingQ(true);
    try {
      const res = await fetch(`${API}/api/quizzes/questions/${selectedQ.id}/`, { method: 'DELETE', headers: authHeader() });
      if (res.ok || res.status === 204) {
        if (expandedQuizId) fetchQuestions(expandedQuizId);
        closeQuestionModal();
  ...
  const filteredReviews = reviews.filter(r => {
    ...
    const matchSearch =
      !q ||
      normalize(r.student_name ?? r.student?.full_name ?? r.student?.username ?? '').includes(q) ||
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/enrollments/admin/users/${selectedUser.id}/certificates/`, {
      headers: authHeader(),   // ← dùng authHeader() như toàn bộ code còn lại
    })
      .then(r => r.json())
      .then(data => setCertificates(Array.isArray(data) ? data : (data.results ?? [])))
          ...
          <div className="cm-box cm-box--sm">
            ...
            <div className="cm-body">
              ...
              <p className="cm-delete-warn">
                ⚠ Hành động này không thể hoàn tác. Tất cả bài học và dữ liệu liên quan sẽ bị xóa vĩnh viễn.
              </p>
              {formError && <p className="cm-error">{formError}</p>}
            </div>
            <div className="cm-footer">
              <button className="cm-btn cm-btn--danger" onClick={handleDelete} disabled={saving}>
                {saving ? '⏳ Đang xóa…' : 'Xóa khóa học'}
              </button>
              <button className="cm-btn cm-btn--cancel" onClick={closeModal} disabled={saving}>Hủy</button>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
        <div className="cm-box">
          <div className="cm-header">
            <h2 className="cm-title">
              <span className={`cm-title-icon cm-title-icon--${isEdit ? 'edit' : 'add'}`}>
                {isEdit ? '✏' : '＋'}
              </span>
          ...
          <div className="cm-body cm-body--scroll">
            ...
            <div className="cm-field">
              ...
              {isEdit && selectedCourse?.thumbnail && !courseForm.thumbnail && (
                <div className="cm-thumb-preview">
                  <img src={selectedCourse.thumbnail} alt="Ảnh bìa hiện tại" className="cm-thumb-img" />
              ...
              {thumbPreviewUrl && (
                <div className="cm-thumb-preview">
                  <img src={thumbPreviewUrl} alt="Xem trước ảnh mới" className="cm-thumb-img" />
                  <button
                    className="cm-thumb-remove"
                    onClick={() => setCourseForm(f => ({ ...f, thumbnail: null }))}
                  >
                    ✕ Bỏ chọn
                  </button>
                </div>
            ...
            <div className="cm-row">
              ...
              <div className="cm-field">
                ...
                <input
                  ...
                  onChange={e => setCourseForm(f => ({ ...f, discount_percent: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="cm-field">
              <label className="cm-label">
                Ngày xuất bản
                <span className="cm-hint"> — để trống nếu chưa lên lịch</span>
              </label>
              <input
                className="cm-input"
                type="datetime-local"
                value={courseForm.published_at}
                onChange={e => setCourseForm(f => ({ ...f, published_at: e.target.value }))}
              />
              {courseForm.published_at && (
                <button
                  className="cm-clear-date"
                  onClick={() => setCourseForm(f => ({ ...f, published_at: '' }))}
                >
                  ✕ Xóa ngày
                </button>
              )}
            ...
            <div className="cm-field cm-field--checkbox">
              <label className="cm-checkbox-label">
                ...
                <span>
                  ...
                  <span className="cm-hint"> — hiển thị trên trang chủ</span>
                </span>
              </label>
            </div>
            ...
            <div className="cm-field">
              ...
              <textarea
                ...
                onChange={e => setCourseForm(f => ({ ...f, what_you_learn: e.target.value }))}
              />
            </div>
          ...
          <div className="cm-footer">
            ...
            <button className="cm-btn cm-btn--cancel" onClick={closeModal} disabled={saving}>
              Hủy
            </button>
          </div>
        </div>
  ...
  const renderSectionModal = () => {
    ...
    if (sectionModal === 'delete') {
      return (
        <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeSectionModal(); }}>
          <div className="cm-box cm-box--sm">
            <div className="cm-header">
              <h2 className="cm-title"><span className="cm-title-icon cm-title-icon--del">🗑</span>Xác nhận xóa chương</h2>
              <button className="cm-close" onClick={closeSectionModal}>✕</button>
            </div>
            <div className="cm-body">
              <p className="cm-delete-desc">Bạn có chắc muốn xóa chương:</p>
              <p className="cm-delete-name">"{selectedSection?.title}"</p>
              <p className="cm-delete-warn">⚠ Tất cả bài học trong chương này cũng sẽ bị xóa.</p>
              {sectionError && <p className="cm-error">{sectionError}</p>}
            </div>
            <div className="cm-footer">
              <button className="cm-btn cm-btn--danger" onClick={handleDeleteSection} disabled={savingSection}>
                {savingSection ? '⏳ Đang xóa…' : 'Xóa chương'}
              </button>
              <button className="cm-btn cm-btn--cancel" onClick={closeSectionModal} disabled={savingSection}>Hủy</button>
            </div>
          </div>
        </div>
      );
        ...
        <div className="cm-box cm-box--sm">
          ...
          <div className="cm-body">
            ...
            <div className="cm-field">
              ...
              <input className="cm-input" type="text" placeholder="Ví dụ: Chương 1 - Giới thiệu"
                value={sectionForm.title}
                onChange={e => setSectionForm(f => ({ ...f, title: e.target.value }))} />
            </div>
            <div className="cm-field">
              <label className="cm-label">Mô tả</label>
              <textarea className="cm-textarea" rows={2} placeholder="Mô tả nội dung chương…"
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--save" onClick={handleSaveSection} disabled={savingSection}>
              {savingSection ? '⏳ Đang lưu…' : isEdit ? 'Lưu thay đổi' : 'Tạo chương'}
            </button>
            <button className="cm-btn cm-btn--cancel" onClick={closeSectionModal} disabled={savingSection}>Hủy</button>
          </div>
        </div>
    ...
    if (lessonModal === 'delete') {
      return (
        <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeLessonModal(); }}>
          <div className="cm-box cm-box--sm">
            <div className="cm-header">
              <h2 className="cm-title"><span className="cm-title-icon cm-title-icon--del">🗑</span>Xác nhận xóa bài học</h2>
            ...
            <div className="cm-section-block">
              ...
              <div className="cm-row">
                ...
                <div className="cm-field">
                  ...
                  {/*  Hiện file mới nếu vừa chọn */}
                  {lessonForm.attachment && (
                    <span className="cm-hint">📎 {lessonForm.attachment.name}</span>
                  )}
                  {/*  Hiện file cũ nếu chưa chọn file mới */}
  ...
  const renderQuizModal = () => {
    ...
    if (quizModal === 'delete') return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeQuizModal(); }}>
        <div className="cm-box cm-box--sm">
          <div className="cm-header">
            <h2 className="cm-title"><span className="cm-title-icon cm-title-icon--del">🗑</span>Xác nhận xóa bài kiểm tra</h2>
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--danger" onClick={handleDeleteQuiz} disabled={savingQuiz}>
        ...
        <div className="cm-box">
          ...
          <div className="cm-body cm-body--scroll">
            <div className="cm-field">
              ...
              <select className="cm-select" value={quizForm.lesson}
                onChange={e => setQuizForm(f => ({ ...f, lesson: e.target.value }))}>
                ...
                {lessons.map(l => {
                  ...
                  return (
                    <option key={l.id} value={l.id}>
                      {course?.title ? `[${course.title}] ` : ''}{sec?.title ? `${sec.title} / ` : ''}{l.title}
                    </option>
                  );
                })}
              </select>
            ...
            <div className="cm-field">
              ...
              <input className="cm-input" type="text" placeholder="Ví dụ: Kiểm tra chương 1"
                value={quizForm.title}
                onChange={e => setQuizForm(f => ({ ...f, title: e.target.value }))} />
            </div>
            <div className="cm-field">
              <label className="cm-label">Hướng dẫn</label>
              <textarea className="cm-textarea" rows={2} placeholder="Mô tả hoặc hướng dẫn làm bài…"
            ...
            <div className="cm-row">
              <div className="cm-field">
                ...
                <input className="cm-input" type="number" min={0} max={100}
                  value={quizForm.pass_score}
                  onChange={e => setQuizForm(f => ({ ...f, pass_score: e.target.value }))} />
              </div>
              <div className="cm-field">
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--save" onClick={handleSaveQuiz} disabled={savingQuiz}>
              {savingQuiz ? '⏳ Đang lưu…' : isEdit ? 'Lưu thay đổi' : 'Tạo bài kiểm tra'}
            </button>
            <button className="cm-btn cm-btn--cancel" onClick={closeQuizModal} disabled={savingQuiz}>Hủy</button>
          </div>
        </div>
    ...
    if (questionModal === 'delete') return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeQuestionModal(); }}>
        <div className="cm-box cm-box--sm">
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--danger" onClick={handleDeleteQuestion} disabled={savingQ}>
              {savingQ ? '⏳…' : 'Xóa'}
            </button>
    ...
    return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeQuestionModal(); }}>
        <div className="cm-box">
          ...
          <div className="cm-body cm-body--scroll">
            <div className="cm-row">
              <div className="cm-field">
                ...
                <select className="cm-select" value={questionForm.question_type}
              ...
              <div className="cm-field">
                ...
                <input className="cm-input" type="number" min={1}
            ...
            <div className="cm-field">
              ...
              <textarea className="cm-textarea" rows={3} placeholder="Nhập câu hỏi…"
                value={questionForm.content}
                onChange={e => setQuestionForm(f => ({ ...f, content: e.target.value }))} />
            </div>
            <div className="cm-field">
              <label className="cm-label">Giải thích đáp án</label>
              <textarea className="cm-textarea" rows={2} placeholder="Giải thích tại sao đáp án đúng…"
            ...
            <div className="cm-field">
              ...
              {questionForm.answers.map((ans, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <input
                    ...
                    onChange={e => {
                      ...
                      if (questionForm.question_type !== 'multiple') {
                        next.forEach((a, i) => { next[i] = { ...a, is_correct: false }; });
                      }
                  ...
                  />
                  <input className="cm-input" type="text" placeholder={`Đáp án ${idx + 1}`}
                    value={ans.content}
                    onChange={e => {
                      const next = [...questionForm.answers];
                  ...
                  {questionForm.answers.length > 2 && (
                    <button onClick={() => {
                      ...
                      style={{ background: 'none', border: 'none', color: '#e07a5f', cursor: 'pointer', fontSize: 16, padding: '0 4px' }}>
                      ✕
                    </button>
                  )}
                </div>
              ))}
              <button onClick={() => setQuestionForm(f => ({
                ...f,
                answers: [...f.answers, { content: '', is_correct: false, order_index: f.answers.length }],
              }))}
                style={{ fontSize: 12, color: '#5ba4de', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0' }}>
                ＋ Thêm đáp án
              </button>
            </div>
            {questionError && <p className="cm-error">{questionError}</p>}
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--save" onClick={handleSaveQuestion} disabled={savingQ}>
              {savingQ ? '⏳ Đang lưu…' : isEdit ? 'Lưu thay đổi' : 'Thêm câu hỏi'}
            </button>
            <button className="cm-btn cm-btn--cancel" onClick={closeQuestionModal} disabled={savingQ}>Hủy</button>
          </div>
        </div>
  ...
  const renderCategoryModal = () => {
    ...
    if (categoryModal === 'delete') return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeCategoryModal(); }}>
        <div className="cm-box cm-box--sm">
          <div className="cm-header">
            <h2 className="cm-title"><span className="cm-title-icon cm-title-icon--del">🗑</span>Xóa danh mục</h2>
            <button className="cm-close" onClick={closeCategoryModal}>✕</button>
          </div>
          <div className="cm-body">
            <p className="cm-delete-desc">Bạn có chắc muốn xóa danh mục:</p>
            <p className="cm-delete-name">"{selectedCategory?.name}"</p>
            <p className="cm-delete-warn">⚠ Các khóa học thuộc danh mục này sẽ bị mất liên kết.</p>
            {categoryError && <p className="cm-error">{categoryError}</p>}
          </div>
          <div className="cm-footer">
            <button className="cm-btn cm-btn--danger" onClick={handleDeleteCategory} disabled={savingCategory}>
              {savingCategory ? '⏳ Đang xóa…' : 'Xóa'}
            </button>
            <button className="cm-btn cm-btn--cancel" onClick={closeCategoryModal} disabled={savingCategory}>Hủy</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="cm-overlay" onClick={e => { if (e.target === e.currentTarget) closeCategoryModal(); }}>
        <div className="cm-box cm-box--sm">
          ...
          <div className="cm-body">
            <div className="cm-field">
              <label className="cm-label">Tên danh mục <span className="cm-required">*</span></label>
              <input className="cm-input" type="text" placeholder="Ví dụ: A1, A2, B1…"
                value={categoryForm.name}
                onChange={e => setCategoryForm(f => ({ ...f, name: e.target.value }))} />
            </div>
            <div className="cm-field">
              <label className="cm-label">Mô tả</label>
              <textarea className="cm-textarea" rows={3} placeholder="Mô tả ngắn về danh mục…"
                value={categoryForm.description}
                onChange={e => setCategoryForm(f => ({ ...f, description: e.target.value }))} />
            </div>
            <div className="cm-row" style={{ alignItems: 'center', gap: 16 }}>
              <div className="cm-field" style={{ flex: 'none' }}>
                <label className="cm-label" style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={categoryForm.is_pinned}
                    onChange={e => setCategoryForm(f => ({ ...f, is_pinned: e.target.checked }))}
              ...
              {categoryForm.is_pinned && (
                <div className="cm-field" style={{ flex: 1 }}>
                  <label className="cm-label">Thứ tự ghim <span style={{ color: 'rgba(224,225,221,0.4)', fontWeight: 400 }}>(1 – 6)</span></label>
                  <input className="cm-input" type="number" min={1} max={6}
                    value={categoryForm.pin_order}
                    onChange={e => setCategoryForm(f => ({ ...f, pin_order: Number(e.target.value) }))} />
                </div>
              )}
            </div>
            {categoryError && <p className="cm-error">{categoryError}</p>}
          ...
          <div className="cm-footer">
            <button className="cm-btn cm-btn--save" onClick={handleSaveCategory} disabled={savingCategory}>
              {savingCategory ? '⏳ Đang lưu…' : isEdit ? 'Lưu thay đổi' : 'Tạo danh mục'}
            </button>
            <button className="cm-btn cm-btn--cancel" onClick={closeCategoryModal} disabled={savingCategory}>Hủy</button>
          </div>
        </div>
        ...
        <div className="ad-modal-overlay" onClick={closeViewUser}>
          <div className="ad-modal ad-modal--user" onClick={e => e.stopPropagation()}>
            <div className="ad-modal__header">
              ...
              <button className="ad-modal__close" onClick={closeViewUser}>✕</button>
            </div>
            <div className="ad-modal__body">
              <div className="ad-modal__user-hero">
                ...
                <div className="ad-modal__user-hero-info">
                  ...
                  <span className={`ad-badge ad-badge--role-${selectedUser.role}`}>
              ...
              <div className="ad-modal__field">
                ...
                <span className="ad-modal__field-value">{selectedUser.email ?? '—'}</span>
              </div>
              <div className="ad-modal__field">
                <span className="ad-modal__field-label">Tên đăng nhập</span>
                <span className="ad-modal__field-value">{selectedUser.username ?? '—'}</span>
              </div>
              <div className="ad-modal__field">
                <span className="ad-modal__field-label">Ngày tham gia</span>
                <span className="ad-modal__field-value">
                  {selectedUser.date_joined ? new Date(selectedUser.date_joined).toLocaleString('vi-VN') : '—'}
                </span>
              </div>
              <div className="ad-modal__field">
                <span className="ad-modal__field-label">Trạng thái</span>
                <span className={`ad-badge ad-badge--${getUserStatus(selectedUser)}`}>
                  {STATUS_LABEL[getUserStatus(selectedUser)] ?? getUserStatus(selectedUser)}
                </span>
              </div>
              {selectedUser.bio && (
                <div className="ad-modal__field">
                  <span className="ad-modal__field-label">Giới thiệu</span>
                  <p className="ad-modal__field-value--comment">{selectedUser.bio}</p>
                </div>
              )}
              ...
              {selectedUser.role === 'student' && (() => {
                ...
                return (
                  <>
                    <div className="ad-modal__section-title">Hồ sơ học viên</div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Số điện thoại</span>
                      <span className="ad-modal__field-value">{sp.phone_number ?? sp.phone ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Ngày sinh</span>
                      <span className="ad-modal__field-value">
                        {sp.date_of_birth ? new Date(sp.date_of_birth).toLocaleDateString('vi-VN') : '—'}
                      </span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Giới tính</span>
                      <span className="ad-modal__field-value">
                        {sp.gender === 'male' ? 'Nam' : sp.gender === 'female' ? 'Nữ' : sp.gender === 'other' ? 'Khác' : '—'}
                      </span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Quốc gia</span>
                      <span className="ad-modal__field-value">{sp.country ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Thành phố</span>
                      <span className="ad-modal__field-value">{sp.city ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Nghề nghiệp</span>
                      <span className="ad-modal__field-value">{sp.occupation ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Học vấn</span>
                      <span className="ad-modal__field-value">
                        {({'high_school':'THPT','college':'Cao đẳng','bachelor':'Đại học','master':'Thạc sĩ','doctor':'Tiến sĩ','other':'Khác'} as any)[sp.education] ?? sp.education ?? '—'}
                      </span>
                    </div>
                    <div className="ad-modal__section-title">Chứng chỉ hoàn thành</div>
                    {loadingCerts ? (
                      <p className="ad-modal__field-value" style={{ opacity: 0.5 }}>Đang tải...</p>
                    ) : certificates.length === 0 ? (
                      <p className="ad-modal__field-value" style={{ opacity: 0.5 }}>Chưa có chứng chỉ nào</p>
                    ) : certificates.map((cert, i) => (
                      <div className="ad-modal__cert-item" key={i}>
                        <div className="ad-modal__cert-name">{cert.course_title}</div>
                        <div className="ad-modal__cert-meta">
                          <span>🎓 {cert.cert_number}</span>
                          <span>📅 {cert.issued_at ? new Date(cert.issued_at).toLocaleDateString('vi-VN') : '—'}</span>
                        </div>
                      </div>
                    ))}
                  </>
              ...
              {selectedUser.role === 'instructor' && (() => {
                ...
                return (
                  <>
                    <div className="ad-modal__section-title">Hồ sơ giảng viên</div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Số điện thoại</span>
                      <span className="ad-modal__field-value">{ip.phone_number ?? ip.phone ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Chức danh</span>
                      <span className="ad-modal__field-value">{ip.title ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Chuyên môn</span>
                      <span className="ad-modal__field-value">{ip.specializations ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Kinh nghiệm</span>
                      <span className="ad-modal__field-value">
                        {ip.years_experience != null ? `${ip.years_experience} năm` : '—'}
                      </span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Chứng chỉ</span>
                      <span className="ad-modal__field-value">{ip.certifications ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Tổng học viên</span>
                      <span className="ad-modal__field-value">{ip.total_students ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Tổng khóa học</span>
                      <span className="ad-modal__field-value">{ip.total_courses ?? '—'}</span>
                    </div>
                    <div className="ad-modal__field">
                      <span className="ad-modal__field-label">Đánh giá TB</span>
                      <span className="ad-modal__field-value">
                        {ip.avg_rating != null ? `${Number(ip.avg_rating).toFixed(1)} ★` : '—'}
                      </span>
                    </div>
                  </>
                );
            ...
            <div className="ad-modal__footer">
              {selectedUser.role !== 'admin' && (
                <button
                  ...
                  onClick={() => { toggleUserStatus(selectedUser); closeViewUser(); }}
                >
                  {getUserStatus(selectedUser) === 'banned' ? 'Mở khóa' : 'Khóa tài khoản'}
                </button>
              )}
              <button className="ad-modal__cancel" onClick={closeViewUser}>Đóng</button>
            </div>
          </div>
        </div>
      )}
        ...
        <main className="ad-main">
                  ...
                  <div className="ad-pending-list">
                    {loadingCourses ? (
                      ...
                      <div key={c.id} className="ad-pending-row">
                        ...
                        <button className="ad-btn-approve" onClick={() => approveCourse(c.id)}>
                          Duyệt
                        </button>
                      </div>
                    ))}
                        ...
                        <tr key={u.id}>
                          ...
                          <td>
                            <div className="ad-action-group">
                              <button className="ad-btn-sm ad-btn-sm--view" onClick={() => openViewUser(u)}>
                                Xem
                              </button>
                              {u.role !== 'admin' && (
                                <button
              ...
              <div className="ad-toolbar">
                <div className="ad-filters">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 13, color: 'rgba(224,225,221,0.4)' }}>Học phí:</span>
                    {[
                      { value: '',     label: 'Tất cả'     },
                  ...
                  {(searchCourse || filterStatus || sortPrice) && (
                    <button
                      ...
                      onClick={() => { setSearchCourse(''); setFilterStatus(''); setSortPrice(''); }}
                    >
                      ✕ Xoá lọc
                    </button>
                  )}
                    ...
                    {loadingCourses ? (
                      ...
                      return (
                        <tr key={c.id}>
                          ...
                          <td>
                            {price === 0 ? 'Miễn phí' : formatPrice(price, 'VND')}
                          </td>
                          <td>
                          ...
                          <td>
                            <div className="ad-actions">
                              <button className="ad-btn-sm ad-btn-sm--edit" onClick={() => openEdit(c)}>
                                Sửa
                              </button>
                              {c.status === 'review' && (
                                <>
                              ...
                              <button className="ad-btn-sm ad-btn-sm--delete" onClick={() => openDelete(c)}>
                                Xóa
                              </button>
                            </div>
                          </td>
                      ...
                      <tr key={s.id}>
                        ...
                        <td>
                          <div className="ad-actions">
                            ...
                            <button className="ad-btn-sm ad-btn-sm--delete" onClick={() => openDeleteSection(s)}>Xóa</button>
                          </div>
                        </td>
                      </tr>
                    ))}
                        ...
                        <tr key={l.id}>
                          ...
                          <td>
                            <div className="ad-actions">
                              ...
                              <button className="ad-btn-sm ad-btn-sm--delete" onClick={() => openDeleteLesson(l)}>Xóa</button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {activeTab === 'quizzes' && (
            <div className="ad-content">
              <div className="ad-page-header">
                ...
                <p className="ad-page-sub">
                  {loadingQuizzes ? 'Đang tải…' : `${quizzes.length} bài kiểm tra`}
                </p>
              </div>
              <div className="ad-toolbar">
                <div className="ad-filters">
                  <select className="ad-select" value={filterQuizLesson}
                    onChange={e => setFilterQuizLesson(e.target.value)}>
                    <option value="">Tất cả bài học</option>
                    {lessons.map(l => <option key={l.id} value={l.id}>{l.title}</option>)}
                  </select>
                  {filterQuizLesson && (
                    <button className="filter-clear" onClick={() => setFilterQuizLesson('')}>✕ Xoá lọc</button>
                  )}
                </div>
                  ...
                  <tbody>
                    {loadingQuizzes ? (
                      <tr><td colSpan={6} style={{ textAlign: 'center' }}>⏳ Đang tải…</td></tr>
                        ...
                        .filter(q => !filterQuizLesson || String(q.lesson?.id ?? q.lesson) === filterQuizLesson)
                        .length === 0 ? (
                      <tr>
                        <td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-text-secondary)' }}>
                              ...
                              <tr>
                                ...
                                <td>
                                  <div className="ad-actions">
                                    ...
                                    <button
                                      className="ad-btn-sm"
                                      style={{ color: '#5ba4de', borderColor: 'rgba(91,164,222,0.3)' }}
                                ...
                                <tr>
                                  <td colSpan={6} style={{ padding: 0 }}>
                                    <div style={{ background: 'rgba(27,38,59,0.6)', padding: '12px 20px', borderTop: '1px solid rgba(119,141,169,0.1)' }}>
                                      {loadingQ ? (
                                        <p className="ad-empty">Đang tải câu hỏi…</p>
                                      ) : questions.length === 0 ? (
                                        <p className="ad-empty">Chưa có câu hỏi nào. Nhấn "＋ Câu hỏi" để thêm.</p>
                                            ...
                                            <div style={{ flex: 1 }}>
                                              ...
                                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                                {ques.answers?.map((a: any) => (
                                                  <span key={a.id} style={{
                                                    fontSize: 12, padding: '2px 8px', borderRadius: 5,
                                                    background: a.is_correct ? 'rgba(76,175,130,0.15)' : 'rgba(255,255,255,0.04)',
                                                    color: a.is_correct ? '#4caf82' : 'rgba(224,225,221,0.5)',
                                      ...
                                      <button className="ad-btn-sm ad-btn-sm--approve" style={{ marginTop: 4 }}
                  ...
                  <tbody>
                    {loadingEnrollments ? (
                      ...
                      const ENROLL_STATUS: Record<string, string> = {
                        active: 'Đang học', completed: 'Hoàn thành', cancelled: 'Đã huỷ',refunded: 'Đã hoàn tiền',
                      };
                      const ENROLL_BADGE: Record<string, string> = {
                        active: 'ad-badge--status-active', completed: 'ad-badge--pay-success', cancelled: 'ad-badge--status-banned',refunded: 'ad-badge--pay-refunded', 
                      };
                      return (
                        <tr key={e.id}>
                          <td>
                          ...
                          <td className="ad-table__muted">
                            {e.enrolled_at ?? e.created_at
                              ? new Date(e.enrolled_at ?? e.created_at).toLocaleDateString('vi-VN')
                              : '—'}
                          </td>
                          <td>
                            {progress !== null ? (
          ...
          {activeTab === 'categories' && (
            <div className="ad-content">
              <div className="ad-page-header">
                ...
                <p className="ad-page-sub">
                  {loadingCategories ? 'Đang tải…' : `${filteredCategories.length} / ${categories.length} danh mục`}
                </p>                
              </div>
              <div className="ad-toolbar">
                <div className="ad-filters">
                  ...
                  />
                  {searchCategory && (
                    <button className="filter-clear" onClick={() => setSearchCategory('')}>✕ Xoá lọc</button>
                  )}
                </div>
                <button className="ad-btn-add-course" onClick={openAddCategory}>＋ Thêm danh mục</button>
              </div>
              <div className="ad-table-wrap">
                <table className="ad-table">
                  <thead>
                  ...
                  <tbody>
                    {loadingCategories ? (
                      ...
                      <tr key={cat.id}>
                        ...
                        <td className="ad-table__muted">{cat.description || '—'}</td>
                        <td className="ad-table__muted">
                          {courses.filter(c => {
                        ...
                        <td>
                          <div className="ad-actions">
                            <button className="ad-btn-sm ad-btn-sm--edit" onClick={() => openEditCategory(cat)}>Sửa</button>
                            <button className="ad-btn-sm ad-btn-sm--ban"  onClick={() => openDeleteCategory(cat)}>Xóa</button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="ad-content">
              ...
              <div className="ad-filters">
                ...
                >
                  ...
                  {[5, 4, 3, 2, 1].map(s => (
                    <option key={s} value={String(s)}>{s} sao</option>
                  ))}
                </select>
                <select
                  className="ad-select"
                ...
                >
                  <option value="">Tất cả khóa học</option>
                  {Array.from(new Set(reviews.map(r => r.course_title ?? r.course?.title ?? ''))).filter(Boolean).map(t => (
                ...
                <table className="ad-table">
                  ...
                  <tbody>
                    {loadingReviews ? (
                      ...
                      <tr>
                        <td colSpan={6} className="ad-empty" style={{ textAlign: 'center', padding: '2rem' }}>
                      ...
                      <tr key={r.id} style={{ opacity: r.is_hidden ? 0.5 : 1 }}>
                        ...
                        <td>
                          <div className="ad-actions">
                            ...
                            <button className="ad-btn-sm ad-btn-sm--ban" onClick={() => openDeleteReview(r)}>Xóa</button>
                          </div>
                        </td>
                      </tr>
                    ))}
                  ...
                  <div className="ad-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
                    ...
                    <div className="ad-modal__body">
                      <div className="ad-modal__field">
                        ...
                        <span className="ad-modal__field-value">
                          {selectedReview.student_name ?? selectedReview.student?.full_name ?? '—'}
                          {(selectedReview.student_email ?? selectedReview.student?.email) && (
                            <span style={{ fontWeight: 400, marginLeft: 6, fontSize: 12, opacity: 0.5 }}>
                              ({selectedReview.student_email ?? selectedReview.student?.email})
                            </span>
                          )}
                        </span>
                      ...
                      <div className="ad-modal__field">
                        ...
                        <span className="ad-modal__field-value">
                          {selectedReview.course_title ?? selectedReview.course?.title ?? '—'}
                        </span>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Số sao</span>
                        <div className="ad-modal__field-value--stars">
                          <span className="ad-modal__stars">{'★'.repeat(selectedReview.rating)}</span>
                          <span className="ad-modal__stars-dim">{'☆'.repeat(5 - selectedReview.rating)}</span>
                          <span className="ad-modal__rating-num">{selectedReview.rating} / 5</span>
                        </div>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Nhận xét</span>
                        {selectedReview.comment
                          ? <p className="ad-modal__field-value--comment">{selectedReview.comment}</p>
                          : <span className="ad-modal__field-value--empty">Không có nhận xét</span>
                        }
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Trạng thái hiển thị</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
                          ...
                          {selectedReview.is_hidden && selectedReview.hidden_at && (
                            <span className="ad-modal__rating-num">
                              từ {new Date(selectedReview.hidden_at).toLocaleDateString('vi-VN')}
                            </span>
                          )}
                          <button
                      ...
                      <div className="ad-modal__meta">
                        ...
                        {selectedReview.edit_count > 0 && (
                          <div className="ad-modal__field">
                            <span className="ad-modal__field-label">Số lần chỉnh sửa</span>
                            <span className="ad-modal__field-value">{selectedReview.edit_count} lần</span>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="ad-modal__footer">
                      <button
                        ...
                        onClick={() => { closeReviewModal(); openDeleteReview(selectedReview); }}
                      >
                        Xóa đánh giá
                      </button>
                ...
                <div className="ad-modal-overlay" onClick={closeReviewModal}>
                  <div className="ad-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
                    ...
                    <p className="ad-modal__delete-desc">
                      ...
                      <strong>{selectedReview.student_name ?? selectedReview.student?.full_name ?? '—'}</strong>{' '}
                      về khóa học{' '}
                      <strong>{selectedReview.course_title ?? selectedReview.course?.title ?? '—'}</strong>?
                    </p>
                    <p className="ad-modal__warn" style={{ margin: '0 22px' }}>⚠️ Hành động này không thể hoàn tác.</p>
                    <div className="ad-modal__footer">
                      <button
                        className="ad-btn-sm ad-btn-sm--ban"
                      ...
                      <button className="ad-modal__cancel" onClick={closeReviewModal}>Hủy</button>
                    </div>
                  </div>
                </div>
              )}
          ...
          {showPaymentModal && (
            <div className="ad-modal-overlay" onClick={closePaymentDetail}>
              <div className="ad-modal ad-modal--payment" onClick={e => e.stopPropagation()}>
                <div className="ad-modal__header">
                  <h2 className="ad-modal__title">Chi tiết giao dịch</h2>
                  <button className="ad-modal__close" onClick={closePaymentDetail}>✕</button>
                </div>
                ...
                <div className="ad-modal__body">
                  {loadingDetail ? (
                    ...
                    <p style={{ textAlign: 'center', color: 'red' }}>Không tải được dữ liệu.</p>
                  ) : (
                    <>
                      {/* ── Trạng thái nổi bật ── */}
                      <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
                        <span className={`ad-badge ad-badge--pay-${paymentDetail.status}`}>
                          {PAYMENT_STATUS_LABEL[paymentDetail.status] ?? paymentDetail.status}
                        </span>
                      </div>

                      {/* ── Mã giao dịch ── */}
                      <div className="ad-modal__section-title">Thông tin giao dịch</div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Mã tham chiếu</span>
                        <span className="ad-modal__field-value" style={{ fontFamily: 'monospace' }}>
                          {paymentDetail.ref_code ?? '—'}
                        </span>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Mã gateway</span>
                        <span className="ad-modal__field-value" style={{ fontFamily: 'monospace' }}>
                          {paymentDetail.gateway_ref || '—'}
                        </span>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Phương thức</span>
                        <span className="ad-modal__field-value">
                      ...
                      <div className="ad-modal__field">
                        ...
                        <span className="ad-modal__field-value" style={{ color: '#4caf82', fontWeight: 600 }}>
                      ...
                      <div className="ad-modal__field">
                        ...
                        <span className="ad-modal__field-value">
                          {paymentDetail.created_at
                            ? new Date(paymentDetail.created_at).toLocaleString('vi-VN')
                            : '—'}
                        </span>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Ngày thanh toán</span>
                        <span className="ad-modal__field-value">
                          {paymentDetail.paid_at
                            ? new Date(paymentDetail.paid_at).toLocaleString('vi-VN')
                            : '—'}
                        </span>
                      </div>
                      ...
                      <div className="ad-modal__field">
                        ...
                        <span className="ad-modal__field-value">{paymentDetail.student_name || '—'}</span>
                      </div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Email</span>
                        <span className="ad-modal__field-value">{paymentDetail.student_email || '—'}</span>
                      </div>

                      {/* ── Khóa học ── */}
                      <div className="ad-modal__section-title">Khóa học</div>
                      <div className="ad-modal__field">
                        <span className="ad-modal__field-label">Tên khóa học</span>
                        <span className="ad-modal__field-value">{paymentDetail.course_title || '—'}</span>
                      </div>
                      ...
                      {paymentDetail.note && (
                        <>
                          ...
                          <p className="ad-modal__field-value--comment">{paymentDetail.note}</p>
                        </>
                      )}
                      {paymentDetail.refund_reason && (
                        <>
                          <div className="ad-modal__section-title">Lý do hoàn tiền</div>
                          <p className="ad-modal__field-value--comment">{paymentDetail.refund_reason}</p>
                        </>
                      )}
                    </>
                  )}
                ...
                <div className="ad-modal__footer">
                  <button className="ad-modal__cancel" onClick={closePaymentDetail}>Đóng</button>
                </div>
              </div>
            </div>
          )}
