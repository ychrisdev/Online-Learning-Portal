...
const MOCK_QUIZ = [
  {
    ...
    correct: 1,
    explanation: '"Have seen" + "before" là cách dùng Present Perfect đúng để nói về kinh nghiệm trong quá khứ không xác định thời điểm.',
  },
  {
    id: 'q2',
    ...
    correct: 1,
    explanation: 'Với chủ ngữ "She" (ngôi thứ 3 số ít) và thói quen hàng ngày, ta dùng thì hiện tại đơn: goes.',
  },
  {
    id: 'q3',
    ...
    correct: 1,
    explanation: '"Eloquent" nghĩa là ăn nói lưu loát, có sức thuyết phục. Ví dụ: "She gave an eloquent speech."',
  },
  {
    id: 'q4',
    ...
    correct: 1,
    explanation: '"Though" phát âm là /ðoʊ/ — âm "th" hữu thanh, vần "-ough" đọc như "oh".',
  },
];
...
type Tab = 'overview' | 'curriculum' | 'lesson' | 'quiz';
          ...
          {activeTab === 'curriculum' && (
            <div className="cd-tab-content cd-curriculum">
              <p className="cd-curriculum__summary">
                {course.curriculum.reduce((a, s) => a + s.lessons.length, 0)} bài học
              ...
              {course.curriculum.map(section => (
                <div key={section.id} className="cd-chapter">
                  ...
                  >
                    ...
                    <span className="cd-chapter__count">{section.lessons.length} bài</span>
                  </button>
                  {expandedSection === section.id && (
                    <div className="cd-chapter__lessons">
                      {section.lessons.map((lesson, idx) => (
                        <button
                          ...
                          onClick={() => { setActiveLessonId(lesson.id); setActiveTab('lesson'); }}
                        >
                          ...
                          {lesson.isPreview && <span className="cd-lesson-row__preview">Xem thử</span>}
                        </button>
                      ))}
                    </div>
                  )}
            ...
            <div className="cd-tab-content cd-lesson">
              {!activeLessonId || !activeLesson ? (
                <div className="cd-lesson__pick">
                  ...
                  <button className="cd-btn-secondary" onClick={() => setActiveTab('curriculum')}>
                ...
                <div className="cd-lesson__content">
                  <div className="cd-lesson__header">
                    <button className="cd-back-sm" onClick={() => setActiveTab('curriculum')}>
                      Danh sách bài
                    </button>
                    <h2 className="cd-lesson__title">{activeLesson.title}</h2>
                    <span className="cd-lesson__duration">{activeLesson.duration}</span>
                  </div>
                  <div className="cd-lesson__body">
                    {activeLesson.content.trim().split('\n').map((line, i) => {
                      ...
                      return <p key={i} className="cd-md-p" dangerouslySetInnerHTML={{ __html: bold }} />;
                    })}
                  </div>
                  <div className="cd-lesson__footer">
                    <button className="cd-btn-enroll" onClick={() => setActiveTab('quiz')}>
            ...
            <div className="cd-tab-content cd-quiz">
              {!quizStarted && !quizDone ? (
                <div className="cd-quiz__intro">
                  ...
                  <button className="cd-btn-enroll" onClick={() => setQuizStarted(true)}>
                    Bắt đầu làm bài
                  </button>
                </div>
              ) : quizDone ? (
                <div className="cd-quiz__result">
                  <div className={`cd-quiz__result-circle ${score >= MOCK_QUIZ.length * 0.7 ? 'cd-quiz__result-circle--pass' : 'cd-quiz__result-circle--fail'}`}>
                    <span className="cd-quiz__result-score">{score}/{MOCK_QUIZ.length}</span>
                    <span className="cd-quiz__result-label">
                      {score >= MOCK_QUIZ.length * 0.7 ? 'Xuất sắc!' : 'Cần ôn thêm'}
                    </span>
                  </div>
                  ...
                  <div className="cd-quiz__result-actions">
                    ...
                    <button className="cd-btn-secondary" onClick={() => setActiveTab('lesson')}>Xem lại bài</button>
                  </div>
                </div>
              ) : (
                <div className="cd-quiz__question">
                  <div className="cd-quiz__progress">
                    <div className="cd-quiz__progress-bar">
                      <div className="cd-quiz__progress-fill" style={{ width: `${(currentQ / MOCK_QUIZ.length) * 100}%` }} />
                    </div>
                    <span className="cd-quiz__progress-text">Câu {currentQ + 1} / {MOCK_QUIZ.length}</span>
                  </div>
                  <h3 className="cd-quiz__q-text">{MOCK_QUIZ[currentQ].question}</h3>
                  <div className="cd-quiz__options">
                    {MOCK_QUIZ[currentQ].options.map((opt, idx) => {
                      let cls = 'cd-quiz__option';
                      if (answered) {
                        if (idx === MOCK_QUIZ[currentQ].correct) cls += ' cd-quiz__option--correct';
                        else if (idx === selected) cls += ' cd-quiz__option--wrong';
                      } else if (idx === selected) {
                        cls += ' cd-quiz__option--selected';
                      }
                      return (
                        <button key={idx} className={cls} onClick={() => handleSelectAnswer(idx)}>
                          <span className="cd-quiz__option-letter">{String.fromCharCode(65 + idx)}</span>
                          {opt}
                        </button>
                      );
                    })}
                  ...
                  {answered && (
                    <div className="cd-quiz__explanation">
                      <strong className={selected === MOCK_QUIZ[currentQ].correct ? 'cd-quiz__correct-text' : 'cd-quiz__wrong-text'}>
                        {selected === MOCK_QUIZ[currentQ].correct ? 'Chính xác!' : 'Chưa đúng.'}
                      </strong>
                      <p>{MOCK_QUIZ[currentQ].explanation}</p>
                    </div>
                  )}
                  {answered && (
                    <button className="cd-btn-enroll cd-quiz__next-btn" onClick={handleNextQuestion}>
                      {currentQ + 1 < MOCK_QUIZ.length ? 'Câu tiếp theo' : 'Xem kết quả'}
                    </button>
                  )}
                </div>
              )}
